...
class Command
  ...
  def implemented_classes?(*class_names)
    class_names.all? { |class_name| Object.const_defined?(class_name) }
  end

  def repository_ready_for_read?(repository)
    repository.respond_to?(:find_all)
  end

  def repository_ready_for_write?(repository)
    repository.respond_to?(:replace_all)
  end
